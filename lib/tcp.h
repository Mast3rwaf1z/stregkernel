...
int tcp_recv_2(struct socket *sock, char *buf, size_t buf_size) {
    struct msghdr msg = {.msg_flags = MSG_WAITALL};
    struct kvec vec = {
        .iov_base = buf,
        .iov_len = buf_size
    };
    kernel_recvmsg(sock, &msg, &vec, 1, buf_size, MSG_WAITALL);
    kernel_recvmsg(sock, &msg, &vec, 1, buf_size, MSG_WAITALL);
    int res = kernel_recvmsg(sock, &msg, &vec, 1, buf_size, MSG_WAITALL);
    return res;
